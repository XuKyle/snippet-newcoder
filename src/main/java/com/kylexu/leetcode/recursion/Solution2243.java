...
public class Solution2243 {
    public String digitSum(String s, int k) {
        ...
        int n = s.length();
        int i = n % k == 0 ? n / k : n / k + 1;
        for (int j = 0; j < i; j++) {
            String substring = s.substring(j * k, Math.min((j + 1) * k, n));
            builder.append(sum(substring));
        }
    ...
    public int sum(String sub) {
        return sub.chars().boxed().map(Character::getNumericValue).mapToInt(Integer::intValue).sum();
    }

    public static void main(String[] args) {
        String s = "00000000";
        int k = 3;
        System.out.println(new Solution2243().digitSum(s, k));
    }
}
